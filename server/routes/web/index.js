module.exports = app => {
  ...
  router.get('/news/init', async(req, res) => {
    ...
    const cats = await Category.find().where({
      parent
    }).lean()   // lean 表示取纯粹的 json 和数组
    const newsTitle = ["新皮肤爆料丨灵猫化身刺客，不知火舞新装登场~", "体验服爆料丨亲密关系升级，战队荣耀再续~", "狄某有话说丨峡谷野区有状况！这种队友必须严惩！", "被CCTV记录的王者荣耀，国人自己的电竞舞台", "新皮肤爆料丨西子湖畔再续前缘，小乔换装化身青蛇~", "3月21日体验服停机修复公告", "3月17日全服不停机更新公告", "实战模拟及困难人机玩法异常提示说明", "3月21日体验服停机更新公告", "【体验服】排位赛、巅峰赛临时关闭调优公告", "峡谷寻宝——传说史诗皮肤自选夺活动开启", "【手Q用户专属】买小乔-青蛇 新皮肤，抽取免单奖励", "春暖花开聚峡谷 多重好礼领不停", "白色情人节【玫瑰芬芳，纸短情长】活动公告", "白色情人节 浪漫好礼来袭", "名嘴之路即将启动！第二届王者荣耀高校解说大赛听你来说", "赛事战令功能开启公告", "2020年KPL春季赛云开赛，今日18点看成都AG和重庆QG揭幕战！", "【KPL今日预报】成都AG超玩会 vs 重庆QG，揭幕战宿敌再遇谁能首胜？", "关于王者荣耀国际轮回赛（KPLGT）积分年的计算规则"]
    const newsList =  newsTitle.map(title => {
      const rendomCats = cats.slice(0).sort((a,b) => Math.random() - 0.5)
      return {
        categories: rendomCats.slice(0, 2),
        title
      }
    })
    await Article.deleteMany({})  // 先清空数据
    await Article.insertMany(newsList)  // 再插入数据
    res.send(newsList)
  })
  ...
  router.get('/news/list', async(req, res) => {
    ...
    const cats = await Category.aggregate([
      ...
      {
        $addFields: {
          newsList: { $slice: ['$newsList', 5] } // 只显示5个
        }
      }
    ])
